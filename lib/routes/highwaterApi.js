...
module.exports = function (userApiClient, kissClient, salt) {
  ...
  return {
    ...
    named_user: function (req, res, next) {
      ...
      let id = hash_id(req.params.userid, 10);
      if((config.metrics == 'all') || (config.metrics == 'file')) {
        metric.info('method[%s], server[%s], user[%s],  event[%s]', req.method, req.url.servername, id, req.params.eventname);
      }
      if((config.metrics == 'all') || (config.metrics == 'kiss')) {
        kissClient.submit(id, req.params.eventname, req.query);
      }
    },
    ...
    token_user: function (req, res, next) {
      ...
      let id = hash_id(req._tokendata.userid, 10);
      if((config.metrics == 'all') || (config.metrics == 'file')) {
          metric.info('method[%s], url[%s], user[%s], event[%s]', req.method, req.url, id, req.params.eventname);
      }
      if((config.metrics == 'all') || (config.metrics == 'kiss')) {
        kissClient.submit(id, req.params.eventname, req.query);
      }
    ...
    server_user: function (req, res, next) {
      ...
      if((config.metrics == 'all') || (config.metrics == 'kiss')) {
        metric.info('method[%s], url[%s], user[server], event[%s]', req.method, req.url, req.params.eventname);        
      }
      if((config.metrics == 'all') || (config.metrics == 'kiss')) {
