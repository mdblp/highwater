...
module.exports = function (userApiClient, kissClient, salt) {
  ...
  function sendMetrics(req, user, hash = true) {
    ...
    if((config.metrics == 'all') || (config.metrics == 'file')) {
      ...
      metric.info(
        ...
        req.method, 
        req.url, 
        (typeof user !== "undefined" && hash) ? hash_id(user, 10) : user, 
        (typeof req.params.patientID !== "undefined" && hash) ? hash_id(req.params.patientID, 10) : req.params.patientID, 
  ...
  return {
    ...
    token_user: function (req, res, next) {
      ...
      sendMetrics(req, req._tokendata.userid);
    },
